...
<template>
  <div style="margin: auto">
    <div style="padding: 1rem" class="container container-fluid no-print">
      ...
      <input id="sheetsNbrInput" placeholder="1" type="number" min="1" max="200" v-model="numberOfSheets" @focusout="handleNbrOfSheetsInput" />
      <label for="fontSizeInput">Rozmiar czcionki:</label>
      <input id="fontSizeInput" min="8" max="32" value="16" type="number" placeholder="16" v-model="inputFontSize" @focusout="handleFontSizeChange($event)"/>
      ...
      <div v-for="(sheet, sheetIndex) in sheets" :key="sheetIndex">
        <ExamPage v-for="(page, pageIndex) in sheet" :index="pageIndex+1" :questions="page" :style="{fontSize : `${fontSize}px`}"
